...
def setup(params):
    ...
    img_shape_x = valid_dataset.img_shape_ns # Data loader is real shape-1 due to legacy thing I didnt want to fix
    img_shape_y = valid_dataset.img_shape_ew
...
def autoregressive_inference(params, ic, valid_data_full, model): 
    ...
    if params.masked_acc:
      maskarray = torch.as_tensor(np.load(params.maskpath)[0:params.img_size[0]]).to(device, dtype=torch.float)
    if params.dfs_type != 'full':
        valid_data = valid_data_full[ic:(ic+prediction_length*dt+n_history*dt):dt, in_channels, 0:params.img_size[0]] #extract valid data from first year
        dfs = sphere_to_torus
    else:
        valid_data = valid_data_full[ic:(ic+prediction_length*dt+n_history*dt):dt, in_channels] # Need pole-> pole for correct full DFS
        dfs = full_sphere_to_torus
    # standardize
    ...
    if orography:
      orog = torch.as_tensor(np.expand_dims(np.expand_dims(h5py.File(orography_path, 'r')['orog'][0:params.img_size[0]], axis = 0), axis = 0)).to(device, dtype = torch.float)
    ...
    with torch.no_grad():
      for i in range(valid_data.shape[0]): 
        if i==0: #start of sequence
          first = valid_data[0:n_history+1].to(device)
          future = valid_data[n_history+1].to(device)
          for h in range(n_history+1):
            print(seq_real.shape, params.img_size)
            seq_real[h] = first[h*n_in_channels : (h+1)*n_in_channels][0:n_out_channels][:, :, :params.img_size[0]].cpu() #extract history from 1st 
            seq_pred[h] = seq_real[h].cpu()
          if params.perturb:
            first = gaussian_perturb(first, level=params.n_level, device=device) # perturb the ic
          if orography:
            future_pred = model(torch.cat((first, orog), axis=1), first.new_tensor([ic+i*dt]))
          else:
            future_pred = model(first, first.new_tensor([ic+i*dt]), extra_out=params.dfs_type=='full')
        else:
          if i < prediction_length-1:
            future = valid_data[n_history+i+1]
          if orography:
            future_pred = model(torch.cat((future_pred, orog), axis=1), first.new_tensor([ic+i*dt])) #autoregressive step
          else:
            future_pred = model(future_pred, first.new_tensor([ic+i*dt]), extra_out=params.dfs_type=='full') #autoregressive step

        if i < prediction_length-1: #not on the last step
          seq_pred[n_history+i+1] = future_pred[:, :, :params.img_size[0]].cpu()
          seq_real[n_history+i+1] = future[:, :params.img_size[0]].cpu()
        ...
        pred = torch.unsqueeze(seq_pred[i], 0).to(device)
        tar = torch.unsqueeze(seq_real[i], 0).to(device)
...
if __name__ == '__main__':
    ...
    vis = args.vis
    print(params.exp_dir, args.config)
    # Set up directory
    if args.override_dir is not None:
      expDir = args.override_dir
      if args.weights is None:
          weights = os.path.join(params.exp_dir, args.config, str(args.run_num))
      else:
    ...
    params['best_checkpoint_path'] = args.weights if args.weights is not None else os.path.join(weights, 'training_checkpoints/best_ckpt.tar')
    ...
    if fld == "z500" or fld == "t850":
        n_samples_per_year = params.samples_per_year #1336
    else:
